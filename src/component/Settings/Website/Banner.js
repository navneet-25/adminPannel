...
const Banner = () => { 
    const { store_banner_list, removeDataToCurrentGlobal, storeBussinessRelode} = useContext(ContextData);
    ...
    const STORY_HEADERS = [

        {
            ...
            cell: (row) => {
                return (
                    <img src={row.image} alt="" style={{ height: '40px', borderRadius: '14px' }} />
                );
            }
        },
            ...
            cell: (row) => { 
                return (
                    <Dropdown>
                        ...
                        <Dropdown.Menu>
                            <Dropdown.Item onClick={() => changeStatus(row.id,row.status)}  >{row.status==1?'Make Not Active':'Make Active'}</Dropdown.Item>
...

        {
            ...
            cell: (row) => { 
                return (
                    <button className="btn btn-danger" onClick={() => deleteAction(row.id,row.image)} ><i className="ri-delete-bin-7-fill me-1 align-bottom" /> Delete  </button>
                    ...
                    fetch(URLDomain + "/APP-API/Billing/deleteStoreBanner", {
                        ...
                        .then((responseJson) => {
                            if (responseJson.delete) {
                                ...
                                getToast({ title: "Deleted ", dec: "Successful", status: "success" });
                    ...
                    <div className="card-body">
                        ...
                        <div className="col-sm-auto ">
                            ...
                            <div className="col-sm-auto ms-auto">
                                <div className="list-grid-nav hstack gap-1">
                                    <button className="btn btn-dark" data-bs-toggle="modal" data-bs-target="#addBrands"><i className="ri-add-fill me-1 align-bottom" /> Add New Banner</button>
                                </div>
                            </div>{/*end col*/}
                        </div>{/*end row*/}
                    </div>
                                ...
                                <div className="modal-dialog modal-dialog-centered w-50">
                                    <div className="modal-content">
                                        <div className="modal-header">
                                            <h5 className="modal-title" id="myModalLabel">Add New Banner</h5>
